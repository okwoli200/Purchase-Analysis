...
IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'PurchaseControl')
	CREATE DATABASE PurchaseControl
ELSE
...
values
		...
		(3, 'StgEmployee.dtsx', 1, 300,1,1, convert(date, getdate()),1),
		(4, 'StgVendor.dtsx', 1, 400,1,1, convert(date, getdate()),1),
		(5, 'stgPurchaseAnalysis.dtsx', 2, 500,1,1, convert(date, getdate()),1),
		(6, 'dimProduct.dtsx', 1, 100,2,1, convert(date, getdate()),1),
		(7, 'dimStore.dtsx', 1, 200,2,1, convert(date, getdate()),1),
		(8, 'dimEmployee.dtsx', 1, 300,2,1, convert(date, getdate()),1),
		(9, 'dimVendor.dtsx', 1, 400,2,1, convert(date, getdate()),1),
		(10, 'factPurchaseAnalysis.dtsx', 2, 1200,2,1, convert(date, getdate()),1)
